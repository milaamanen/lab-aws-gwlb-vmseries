...
```
git clone https://github.com/milaamanen/lab-aws-gwlb-vmseries.git && cd lab-aws-gwlb-vmseries/terraform/
```

## 3.8. Deploy Panorama and TGW Infrastructure

1. Go to the lab folder/terraform/panorama
2. Deploy Panorama and Transit Gateway with commands:
  - terraform init
  - terraform apply
This step takes 7-10 minutes
3. Copy Terraform Output values to your notepad/text editor

## 3.9. Prepare Panorama

1. Login to the Panorama with your web browser with URL what you copied from the last step.
2. Go to Panorama -> Setup -> Interfaces -> Click on Management interface name to edit.
3. Add public IP for the interface and click Ok.
4. Commit changes to Panorama on top right corner.
...
Add SSH key name and Panorama public IP-address to the student.auto.tfvars file.
```
ssh_key_name        = ""
panorama_host       = ""
...
> &#8505; We are using Panorama for this lab that is publicly accessible. For production deployments, Panorama management should not be exposed to inbound Internet traffic.
